...
// smaller noise
float offset = 0.0;
float rotation = snoise(vec3(sPos.x - u_time * 0.08 + offset, sPos.y + offset, sPos.z * 50.0 + u_time * 0.04 + offset)) * PI * 0.5;
offset = 0.5;
float distance = snoise(vec3(sPos.x - u_time * 0.08 + offset, sPos.y + offset, sPos.z * 50.0 + u_time * 0.04 + offset)) * 2.0;
